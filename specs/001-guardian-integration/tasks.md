...
- [ ] T009 [P] Implement `CircuitBreaker` state machine (closed / open / half_open / tos_required) in [api/app/service/guardian_client.py](api/app/service/guardian_client.py) per research.md §3: 3-strikes counter, 60 s window, single-probe half-open, `clock` injection for tests. `record_success()` / `record_failure(error_class)` / `should_allow_call()` public API.
- [ ] T010 Implement `GuardianClient.__init__`, `login()`, `get_health()`, `circuit_status()` in [api/app/service/guardian_client.py](api/app/service/guardian_client.py). Login caches the JWT; 401 triggers re-login; 451 sets breaker to `tos_required` (no counter increment).
- [ ] T011 Implement identity methods `register_user()` (409-idempotent per FR-008), `set_user_credentials()`, `get_user_did()` in [api/app/service/guardian_client.py](api/app/service/guardian_client.py). Depends on T010.
- [ ] T012 Implement `wait_for_task(task_id, timeout=120.0)` with `tenacity` exponential backoff (500 ms init, factor 2, cap 8 s) in [api/app/service/guardian_client.py](api/app/service/guardian_client.py). Depends on T010.
- [ ] T013 [P] Add `respx` MGS-mock fixture `mgs_mock` to [api/app/tests/conftest.py](api/app/tests/conftest.py) that hooks `httpx.AsyncClient`, and a `frozen_clock` fixture for breaker tests.
- [ ] T014 [P] Remove the skip guard at the top of [api/app/tests/integration/guardian/conftest.py](api/app/tests/integration/guardian/conftest.py) now that `respx` and `guardian_client` resolve; the 45 scaffolded tests flip from SKIP → FAIL. Commit and confirm RED state before any further implementation (Constitution §V).
- [ ] T015 Implement `GET /guardian/health` in [api/app/routes/guardian/identity.py](api/app/routes/guardian/identity.py) returning `{status: ok | tos_required | breaker_open | unavailable, breaker: <state>, last_failure_at: ...}` per FR-009. Depends on T010.
- [ ] T016 Implement `POST /guardian/register` (FR-008 idempotent) and `GET /guardian/did/{username}` in [api/app/routes/guardian/identity.py](api/app/routes/guardian/identity.py). Depends on T011.
- [ ] T017 Add explicit operator-consent capture on `POST /guardian/register` (FR-015): request body requires `acknowledged_vc_disclosure: bool = True` + `consent_recorded_at` timestamp; reject with 400 if absent. Consent is persisted in the existing operator record; no new table.
- [ ] T018 Update [api/app/tests/integration/guardian/test_guardian_client_contract.py](api/app/tests/integration/guardian/test_guardian_client_contract.py) to assert the 6 foundational contract invariants (login/401-refresh, 451→`tos_required`, 409→idempotent register, task polling timeout, breaker state transitions with injected clock, /guardian/health discriminates all four statuses). Tests must PASS against T008–T016.
