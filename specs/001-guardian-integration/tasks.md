...
- [X] T036 [US1] Implement `GET /guardian/gdst/vc/{event_hash}` with `?history=true` support in [api/app/routes/guardian/policy.py](api/app/routes/guardian/policy.py); responses follow FR-007 / SC-007 (200 / 202 pending / 404 / 503 `vc_manual_review`). Depends on T032, T033. — NOTE: v1 surfaces only 200/404 via HTTP; the 202/503 pending/manual-review flow is exposed through `GuardianClient.get_vc_retrieval_status()` direct-caller contract (see policy.py docstring).
- [ ] T036a [US1/US2] Introduce the `PolicyConfig` registry per [data-model.md §Policy Registry](specs/001-guardian-integration/data-model.md) and [research.md §8](specs/001-guardian-integration/research.md). Concrete changes:
