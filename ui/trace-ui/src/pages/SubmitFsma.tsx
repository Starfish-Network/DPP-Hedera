...
export function SubmitFsma() {
    const sampleKeys = Object.keys(fsmaSamples).sort((a, b) => a.localeCompare(b));
    ...
    return (
        <div className="space-y-4">
            ...
            {shouldPoll && pollState.status === "ready" && (
                <GuaranteedFieldsCard vc={pollState.vc} slug="fsma" />
            )}
        </div>
    );
}
...
function GuardianStatusInline({
    ...
    if (guardian.status === "skipped") {
        const REASONS: Record<typeof guardian.reason, string> = {
            not_configured:
                "policy not configured — run scripts/build_fsma_policy.py and populate api/.env.dev",
            not_compliant:
                "rule predicate failed (fsma_min_rules returned False) — no VC will be issued",
            breaker_open:
